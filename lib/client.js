...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Perceptual curve: slider value (0..80) -> alpha. The exponent boosts the
     *  low range so small transparency values produce a visible change and the
     *  drag feels uniform (perceived translucency is non-linear in alpha). */
    function transparencyAlpha(v) {
      var t = Math.max(0, Math.min(80, Number(v) || 0)) / 80;
      return 1 - 0.8 * Math.pow(t, 0.85);
    }
