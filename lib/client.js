...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    exports.apply = apply;
    // NOTE: "theme" is deliberately NOT declared here — a missing service in
    // the inject list fails plugin ACTIVATION (killing even the background).
    // We resolve it lazily via ctx.get with a guard, so a composition without
    // the theme service degrades gracefully instead of breaking everything.
