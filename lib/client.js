...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var LS_BRIGHTNESS = "dsh-animated-bg.brightness";   // 1 = as-is, >1 brighter
    var LS_TONE = "dsh-animated-bg.tone";   // "off" | "auto" | preset id
    var LS_SIDEBAR_ALPHA = "dsh-animated-bg.sidebarAlpha";   // 1 = opaque, 0.2 = most transparent
    var LS_SURFACE_ALPHA = "dsh-animated-bg.surfaceAlpha";   // main-surface alpha
    ...
    function applyMedia(kind, media, mask) {
      ...
      var m = typeof mask === "number" ? mask : 0.15;
      var bright = Number(loadLS(LS_BRIGHTNESS, "1")) || 1;
      if (kind === "video") {
        removeBgElement();
        if (!videoEl) {
        ...
        videoEl.style.filter = "brightness(" + bright + ")";
        if (!maskEl) {
    ...
    /** Brightness live-tune: re-apply the filter to the current media layer. */
    function setBrightness(brightness) {
      var b = Number(brightness) || 1;
      saveLS(LS_BRIGHTNESS, String(b));
      if (videoEl) { try { videoEl.style.filter = "brightness(" + b + ")"; } catch (e) { } }
      if (bgEl) { try { bgEl.style.filter = "brightness(" + b + ")"; } catch (e) { } }
    }
    ...
    function applyCurrent() {
      // translucent surfaces while a background is enabled (so the wallpaper
      // shows through the app frame); cleared when disabled
      applySurfaces(loadLS(LS_ENABLED, "0") === "1");
      var mask = Number(loadLS(LS_MASK, "0.15"));
    ...
    var toneBusy = false;        // re-entrancy guard for theme/change resync
    var toneResyncTimer = null;
    var surfacesDisposer = null;

    /**
     * Make the app surfaces translucent while a background is enabled. The
     * wallpaper layer sits at z-index:-1, and the app frame/content paints
     * opaque surfaces over the whole viewport — without this, the background
     * is invisible. Translucent bg-base / bg-layer-1 let it glow through while
     * foreground text (painted above) stays readable.
     */
    function applySurfaces(on) {
      if (surfacesDisposer) { try { surfacesDisposer(); } catch (e) { } surfacesDisposer = null; }
      if (!on || !themeService) return;
      var a = Number(loadLS(LS_SURFACE_ALPHA, "0.5"));
      if (isNaN(a)) a = 0.5;
      a = Math.max(0.2, Math.min(1, a));
      var aL1 = Math.min(1, a + 0.08);   // bars keep a touch more contrast
      try {
        surfacesDisposer = themeService.overrideTokens("dsh-thirteen-bg-surfaces", {
          "--dsw-alias-bg-base": { light: "rgba(248, 250, 252, " + a + ")", dark: "rgba(10, 13, 20, " + a + ")" },
          "--dsw-alias-bg-layer-1": { light: "rgba(255, 255, 255, " + aL1 + ")", dark: "rgba(17, 20, 27, " + aL1 + ")" }
        });
      } catch (e) { surfacesDisposer = null; }
    }

    /** Set the main-surface translucency (0 = opaque, 0.8 = most transparent). */
    function setSurfaceTransparency(transparency) {
      var alpha = transparencyAlpha(transparency);
      saveLS(LS_SURFACE_ALPHA, String(alpha));
      applySurfaces(loadLS(LS_ENABLED, "0") === "1");
    }

    /** Debounce rapid chrome changes (slider drags): apply the theme override
     *  layer only after the gesture settles, avoiding a theme/change re-render
     *  storm per tick that can freeze the UI and break the slider drag. */
    function scheduleChromeApply(fn, ms) {
     ...
     *  Deliberately does NOT touch the --dsw-alias-bg-* page/surface tokens: the
     *  app frame paints them full-viewport, and overriding them would cover the
     *  wallpaper layer. Only the chrome (borders, bars, accents, hover, subtle
     *  text) is tinted. */
    ...
    /** Apply (or clear) the chrome tone + sidebar translucency.
     *  mode: "off" | "auto" | preset id. The sidebar alpha is read from LS and
     *  composed into the same override layer, so both features stay in sync. */
    function applyTone(mode, cb) {
      var alpha = Number(loadLS(LS_SIDEBAR_ALPHA, "1"));
      var finish = function (primary) {
        if (toneDisposer) { try { toneDisposer(); } catch (e) { } toneDisposer = null; }
        var overrides = primary ? toneOverridesFor(primary) : {};
        if (alpha < 0.999) {
          var sideLight = primary ? mix("#ffffff", primary, 0.10) : "#f9fafb";
          var sideDark = primary ? mix("#0d1320", primary, 0.24) : "#1b1b1c";
          overrides["--dsw-specific-sidebar-fill"] = {
            light: withAlpha(sideLight, alpha),
            dark: withAlpha(sideDark, alpha)
          };
        }
        ...
        try {
          toneBusy = true;
          toneDisposer = themeService.overrideTokens(TONE_SOURCE, overrides);
          toneBusy = false;
          toneStatus = primary || "sidebar";
          if (cb) cb(primary || "sidebar");
        } catch (e) { toneBusy = false; toneStatus = ""; if (cb) cb(null); }
      };
      saveLS(LS_TONE, mode);
    ...
    /** Perceptual curve: slider value (0..0.8 FRACTION) -> alpha. The exponent
     *  boosts the low range so small transparency values produce a visible
     *  change and the drag feels uniform (perceived translucency is
     *  non-linear in alpha). v is normalized against the slider max (0.8). */
    function transparencyAlpha(v) {
      var t = Math.max(0, Math.min(0.8, Number(v) || 0)) / 0.8;   // 0..1
      return 1 - 0.8 * Math.pow(t, 0.85);                          // alpha 1..0.2
    }

    /** Set the sidebar transparency (0 = opaque, 80 = most transparent). */
    function setSidebarTransparency(transparency) {
      var alpha = transparencyAlpha(transparency);
      saveLS(LS_SIDEBAR_ALPHA, String(alpha));
      applyTone(loadLS(LS_TONE, "off"));
    }
    ...
    /**
     * theme/change resync: other plugins (e.g. skin packs) may re-stack their
     * own token layers after ours during boot, covering the translucent
     * sidebar. Re-applying our layer (same source restacks it on top) keeps
     * the chrome override authoritative. Debounced + guarded against the
     * change our own overrideTokens emits.
     */
    function resyncChrome() {
      if (toneBusy) return;
      if (toneResyncTimer) { clearTimeout(toneResyncTimer); }
      toneResyncTimer = setTimeout(function () {
        toneResyncTimer = null;
        var mode = loadLS(LS_TONE, "off");
        var alpha = Number(loadLS(LS_SIDEBAR_ALPHA, "1"));
        if (mode !== "off" || alpha < 0.999) {
          applyTone(mode);
        }
      }, 120);
    }
    ...
    var en = {
      ...
      sidebarTrans: "Sidebar translucency",
      surfaceTrans: "Surface translucency",
    ...
    function AnimatedBgRow(props) {
      ...
      var state = React.useState(function () {
        return {
          ...
          url: loadLS(LS_URL, ""),
          mask: Number(loadLS(LS_MASK, "0.15")),
          brightness: Number(loadLS(LS_BRIGHTNESS, "1")),
          tone: loadLS(LS_TONE, "off"),
          sidebarTrans: 1 - Number(loadLS(LS_SIDEBAR_ALPHA, "1")),
          surfaceTrans: 1 - Number(loadLS(LS_SURFACE_ALPHA, "0.5")),
          status: ""
        };
      });
      var cfg = state[0];
      ...
      function patch(next, status) {
        // functional updater: async callbacks (applyCurrent etc.) must not
        // revert fields using a stale closure `cfg`
        setCfg(function (prev) {
          return Object.assign({}, prev, next, { status: status === undefined ? prev.status : status });
        });
      }
      ...
      function onSidebarTrans(ev) {
        var trans = Number(ev.target.value);
        // local state updates live; the theme layer applies after the drag settles
        patch({ sidebarTrans: trans });
        scheduleChromeApply(function () {
          setSidebarTransparency(trans);
          patch({}, t("statusOk"));
        });
      }

      function onSurfaceTrans(ev) {
        var trans = Number(ev.target.value);
        patch({ surfaceTrans: trans });
        scheduleChromeApply(function () {
          setSurfaceTransparency(trans);
          patch({}, t("statusOk"));
        });
      }

      function onBrightness(ev) {
        var b = Number(ev.target.value);
        patch({ brightness: b });
        setBrightness(b);
      }
      ...
      return React.createElement(
        ...
        React.createElement(
          "div", { className: "dsh-animated-bg-row" },
          React.createElement("span", null, t("sidebarTrans")),
          React.createElement("input", {
            type: "range",
            min: "0",
            max: "0.8",
            step: "0.01",
            value: cfg.sidebarTrans,
            onChange: onSidebarTrans
          }),
          React.createElement("span", null, Math.round((1 - transparencyAlpha(cfg.sidebarTrans)) * 100) + "%")
        ),
        React.createElement(
          "div", { className: "dsh-animated-bg-row" },
          React.createElement("span", null, t("surfaceTrans")),
          React.createElement("input", {
            type: "range",
            min: "0",
            max: "0.8",
            step: "0.01",
            value: cfg.surfaceTrans,
            onChange: onSurfaceTrans
          }),
          React.createElement("span", null, Math.round((1 - transparencyAlpha(cfg.surfaceTrans)) * 100) + "%")
        ),
        React.createElement(
          "div", { className: "dsh-animated-bg-row" },
          React.createElement("span", null, t("brightness")),
          React.createElement("input", {
            type: "range",
            min: "0.6",
            max: "1.8",
          ...
          React.createElement("span", null, Math.round(cfg.brightness * 100) + "%")
        ),
    ...
    function apply(ctx) {
      ensureStyle();
      try { themeService = ctx.get("theme"); } catch (e) { themeService = null; }
      // restore the saved background at activation (not lazily from the card)
      var bgReady = applyCurrent();

      // restore the saved chrome tone + sidebar translucency (auto mode
      // samples the media once it is up). Trigger even when only the sidebar
      // translucency is set (tone left at its default "off").
      var savedTone = loadLS(LS_TONE, "off");
      var savedAlpha = Number(loadLS(LS_SIDEBAR_ALPHA, "1"));
      if (savedTone !== "off" || savedAlpha < 0.999) {
        if (savedTone === "auto") {
      ...
      // keep the chrome override on top when other plugins re-stack theme layers
      if (ctx.on) { ctx.on("theme/change", resyncChrome); }
      ...
      return function () {
        ...
        if (toneResyncTimer) { clearTimeout(toneResyncTimer); toneResyncTimer = null; }
        if (ctx.off) { try { ctx.off("theme/change", resyncChrome); } catch (e) { } }
        if (surfacesDisposer) { try { surfacesDisposer(); } catch (e) { } surfacesDisposer = null; }
        clearTone();
        themeService = null;
        removeAllLayers();
    ...
    exports.apply = apply;
    // "theme" must stay declared here: the runtime gates ctx.get() on the
    // inject list, so tone/sidebar need it. Independence from skin plugins
    // comes from OUR manifest edge (dsh.client.inject -> ui-theme) being in
    // the boot graph, not from dropping the declaration. ctx.get stays
    // try/catch-guarded.
